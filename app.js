...
async function handleFile(e, mode) {
  ...
  try {
    ...
    if (mode === 'actual') {
      ...
      const label = `P${S.historico.length + 1} · ${new Date().toLocaleDateString('es-AR',{day:'2-digit',month:'2-digit'})}`;
      S.historico.push({ label, filename: file.name, rows });
...
function renderHistorico() {
  ...
  const periods = S.historico.map(p => {
    ...
    return { label: p.label, filename: p.filename, n: rows.length, activos, aum: aumT, com: comT, roa };
  });
  ...
  // charts
  drawLineChart('chartAum',     periods.map(p=>p.label), periods.map(p=>p.aum),     'var(--blue)',  'AUM USD');
  drawLineChart('chartFact',    periods.map(p=>p.label), periods.map(p=>p.com),     'var(--green)', 'Comisión 180');
  drawLineChart('chartActivos', periods.map(p=>p.label), periods.map(p=>p.activos), 'var(--amber)', 'Clientes activos');
}
...
function renderScatterChart(rows) {
  ...
  const valid = rows.filter(r => r.aum > 0 && r.roa !== null);
...
function drawLineChart(containerId, labels, values, color, label) {
  ...
  const W=560, H=160, PL=60, PR=20, PT=12, PB=28;
  const IW=W-PL-PR, IH=H-PT-PB;
  const minV = Math.min(...values);
  const maxV = Math.max(...values);
  const range = maxV - minV || 1;
  const xPos  = i => PL + (i/(values.length-1))*IW;
  const yPos  = v => PT + IH - ((v-minV)/range)*IH;

  const pts    = values.map((v,i) => `${xPos(i).toFixed(1)},${yPos(v).toFixed(1)}`).join(' ');
  const area   = `${PL},${PT+IH} ` + values.map((v,i) => `${xPos(i).toFixed(1)},${yPos(v).toFixed(1)}`).join(' ') + ` ${xPos(values.length-1)},${PT+IH}`;

  const isNum  = typeof values[0] === 'number' && values[0] > 100;
  const fmtVal = v => isNum ? fmtUSD(v) : fmt(v);

  const dots   = values.map((v,i) => `
    <circle cx="${xPos(i).toFixed(1)}" cy="${yPos(v).toFixed(1)}" r="4" fill="${color}" stroke="var(--s1)" stroke-width="2">
      <title>${labels[i]}: ${fmtVal(v)}</title>
    </circle>`).join('');

  const xlabs  = labels.map((l,i) =>
    `<text x="${xPos(i).toFixed(1)}" y="${H-6}" text-anchor="middle" fill="var(--text3)" font-size="10">${l}</text>`).join('');
  ...
  el.innerHTML = `<svg viewBox="0 0 ${W} ${H}" style="width:100%" xmlns="http://www.w3.org/2000/svg">
    ...
    ${dots}
    ${xlabs}
    <text x="${PL-6}" y="${PT+4}" text-anchor="end" fill="var(--text3)" font-size="9">${fmtVal(maxV)}</text>
    <text x="${PL-6}" y="${PT+IH}" text-anchor="end" fill="var(--text3)" font-size="9">${fmtVal(minV)}</text>
  </svg>`;
}
